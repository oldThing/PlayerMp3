...
public class PlayCtrlBarPagerAdapter extends PagerAdapter {
    ...
    private void bindData(View v, int position) {
        TextView songName = (TextView) v.findViewById(R.id.tv_song_name);
        songName.setText("Try - " + position);

        ImageView artistImage = (ImageView) v.findViewById(R.id.iv_artist_cover);
        if (position % 2 == 1) {
//            artistImage.setImageResource(R.mipmap.adele);
        } else {
//            artistImage.setImageResource(R.mipmap.bxbxbai);
        }
    }
